...
const logger = require('pino')({ name: 'New Event', level: 'info' });
...
try{
    ...
    const { notificationEvent, emails } = body;

    const templateData = JSON.stringify({
        artist: notificationEvent.artistName,
        venue: notificationEvent.venueName,
        datetime: notificationEvent.sortDate
    })
    var emailResponse = {};
    for(var i = 0; i < emails.length; i++){
        let askerDetails = await getCognitoUserDetails(emails[i]);
        let toEmail = await getAttributeFromUserDetails(askerDetails, "email");
        emailResponse += await sendEmail(toEmail, templateData);
    }
